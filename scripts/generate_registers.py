...
GLIB_IPB_BASE_ADDRESS = 0x01000000
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print('num_of_oh is optional. if supplied we will substitute CFG_NUM_OF_OH variable with the given number. Otherwise it will be given a default value: 4 for CTP7 and 2 for GLIB')
        return
    else:
...
def findRegisters(node, baseName, baseAddress, modules, currentModule, vars, isGenerated):
    if (isGenerated == None or isGenerated == False) and node.get('generate') is not None and node.get('generate') == 'true':
        if node.get('generate_size') == '${CFG_NUM_OF_OH}':
