...
class Prompt(Cmd):
                ...
                if getNodesContaining('GEM_AMC.OH.OH'+str(arglist[0])) is not None:
                    for reg in getNodesContaining('GEM_AMC.OH.OH'+str(arglist[0])):
                        ...
                        if 'r' in str(reg.permission):
                            print hex(address).rstrip('L'),reg.permission,'\t',tabPad(reg.name,7),readReg(reg,glib)
    ...
    def do_read(self, args):
        ...
        if reg is not None: 
            address = reg.real_address
            # if 'r' in str(reg.permission):
            #     print displayReg(reg,None,glib)
    ...
    def do_readGroup(self, args): #INEFFICIENT
        ...
        if node is not None: 
            ...
            for reg in kids: 
                if 'r' in str(reg.permission): print displayReg(reg,None,glib)
    ...
    def do_readFW(self, args):
        ...
        for reg in getNodesContaining('STATUS.FW'):
            if 'r' in str(reg.permission): print hex(reg.real_address),reg.permission,'\t',tabPad(reg.name,4),readReg(reg,glib)
    ...
    def do_readAddress(self, args):
        ...
        if reg is not None: 
            address = reg.real_address
            if 'r' in str(reg.permission):
                print hex(address),'{0:#010x}'.format(reg.mask),reg.permission,'\t',reg.name,'\t',readReg(reg,glib)
            elif reg.isModule: print 'This is a module!'
            else: print hex(address),'\t',reg.name,'\t','No read permission!' 
        else:
            print args,'not found!' 
    ...
    def do_debug(self,args):
        ...
        if len(arglist)==1:
            for reg in getNodesContaining('OH'+str(args)+'.DEBUG_LAST'):
                try: cluster = parseInt(readReg(reg,glib))
                except: cluster = 0
                if cluster == 2047:
                    if 'r' in str(reg.permission): print hex(reg.real_address),reg.permission,'\t',tabPad(reg.name,4),readReg(reg,glib),'(None)'
                else:
                    if 'r' in str(reg.permission): print hex(reg.real_address),reg.permission,'\t',tabPad(reg.name,4),readReg(reg,glib),'(VFAT:',cluster_to_vfat(cluster),'SBit:',cluster_to_vfat2_sbit(cluster),'Size:',cluster_to_size(cluster),')'
...
if __name__ == '__main__':
    try:
        ...
        prompt = Prompt()
        prompt.prompt = 'GLIB > '
        prompt.cmdloop('Starting GLIB Register Command Line Interface.')
